...
def preprocess(text):
	...
	#text = re.sub(r'#\S+', split_hashtag, text) # 1st version Split hashtags on uppercase letters
	#text = re.sub(r'#([^\S]+)', r'\1', text) # 2nd version
	text = re.sub(r'#([^\S]+)', split_hashtag, text) # 3rd version  
